	...
	if agent.Spec.ClusterDeploymentName == nil && h.ClusterID != nil {
		...
		if funk.ContainsString(installingStatuses, *h.Status) {
			if swag.StringValue(h.Kind) != models.HostKindAddToExistingClusterHost {
				...
				err := fmt.Errorf("cancelling non-day-2 host [%s] in the middle of installation is not allowed", *h.ID)
				log.Error(err)
				return r.updateStatus(ctx, log, agent, origAgent, &h.Host, h.ClusterID, err, false)
			}
			// Host is in the middle of installing so cancel the installation first to allow moving to unbind
			newHost, err := r.Installer.CancelDay2HostInstallationInternal(ctx, h)
			if err != nil {
				log.WithError(err).Errorf("failed cancelling installation for host %s", *h.ID)
